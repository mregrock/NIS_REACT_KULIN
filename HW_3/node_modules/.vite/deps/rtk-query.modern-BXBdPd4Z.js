import { $ as produce, C as createSlice, D as isAllOf, F as isRejected, G as weakMapMemoize, H as createSelector, I as isRejectedWithValue, J as enablePatches, K as applyPatches, N as isPending, O as isAnyOf, Q as original, R as nanoid, X as isDraft, Z as isDraftable, ct as isPlainObject, et as produceWithPatches, f as createAction, it as combineReducers, j as isFulfilled, k as isAsyncThunkAction, m as createAsyncThunk, n as SHOULD_AUTOBATCH, q as current, st as isAction, z as prepareAutoBatched } from "./redux-toolkit.modern-DDnGPW1X.js";
...
var timeoutSignal = (milliseconds) => {
	...
	setTimeout(() => {
		...
		abortController.abort(typeof DOMException !== "undefined" ? new DOMException(message, name) : Object.assign(new Error(message), { name }));
	}, milliseconds);
...
var retryWithBackoff = (baseQuery, defaultOptions) => async (args, api, extraOptions) => {
	...
	while (true) {
		...
		try {
			...
			} else if (retry2 > options.maxRetries) return { error: e };
			failIfAborted(api.signal);
			try {
...
function buildThunks({ reducerPath, baseQuery, context: { endpointDefinitions }, serializeQueryArgs, api, assertTagType, selectors, onSchemaFailure, catchSchemaFailure: globalCatchSchemaFailure, skipSchemaValidation: globalSkipSchemaValidation }) {
	...
	const executeEndpoint = async (arg, { signal, abort, rejectWithValue, fulfillWithValue, dispatch, getState, extra }) => {
		...
		try {
			...
			if (typeof process !== "undefined" && true) console.error(`An unhandled error occurred processing a request for the endpoint "${arg.endpointName}".
		...
		return createAsyncThunk(`${reducerPath}/executeQuery`, executeEndpoint, {
			getPendingMeta({ arg }) {
				...
				return addShouldAutoBatch({
					...
					...isInfiniteQueryDefinition(endpointDefinition) ? { direction: arg.direction } : {}
				});
			},
...
var defaultSerializeQueryArgs = ({ endpointName, queryArgs }) => {
	...
	if (typeof cached === "string") serialized = cached;
		...
		if (isPlainObject(queryArgs)) cache?.set(queryArgs, stringified);
		...
		function injectEndpoints(inject) {
			...
			for (const [endpointName, definition] of Object.entries(evaluatedEndpoints)) {
				if (inject.overrideExisting !== true && endpointName in context.endpointDefinitions) {
					if (inject.overrideExisting === "throw") throw new Error(`called \`injectEndpoints\` to override already-existing endpointName ${endpointName} without specifying \`overrideExisting: true\``);
					else if (typeof process !== "undefined" && true) console.error(`called \`injectEndpoints\` to override already-existing endpointName ${endpointName} without specifying \`overrideExisting: true\``);
					continue;
				}
				if (typeof process !== "undefined" && true) {
					if (isInfiniteQueryDefinition(definition)) {
						...
						if (typeof maxPages === "number") {
							if (maxPages < 1) throw new Error(`maxPages for endpoint '${endpointName}' must be a number greater than 0`);
							if (typeof getPreviousPageParam2 !== "function") throw new Error(`getPreviousPageParam for endpoint '${endpointName}' must be a function if maxPages is used`);
						}
					}
				}
...
var buildDevCheckHandler = ({ api, context: { apiUid }, reducerPath }) => {
	return (action, mwApi) => {
		...
		if (typeof process !== "undefined" && true) {
			if (api.internalActions.middlewareRegistered.match(action) && action.payload === apiUid && mwApi.getState()[reducerPath]?.config?.middlewareRegistered === "conflict") console.warn(`There is a mismatch between slice and middleware for the reducerPath "${reducerPath}".
...
//#endregion
export { copyWithStructuralSharing as a, createApi as c, fetchBaseQuery as d, retry as f, buildCreateApi as i, defaultSerializeQueryArgs as l, skipToken as m, QueryStatus as n, coreModule as o, setupListeners as p, _NEVER as r, coreModuleName as s, NamedSchemaError as t, fakeBaseQuery as u };
//# sourceMappingURL=rtk-query.modern-BXBdPd4Z.js.map